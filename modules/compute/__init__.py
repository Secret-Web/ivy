        ...
        async def event(packet):
            ...
            for id, data in packet.payload.items():
                ...
                if await self.database.machines.has(id):
                    ...
                    # This update was pushed by a miner, assume its group is wrong
                    if not isinstance(packet.sender, int):
                        if 'config' in data:
                            data['config']['group'] = client.config.group.as_obj(is_client=True)
                else:
                    client = Client()

                client.update(**data)

                # TODO: Watchdog for client gpu counts
    ...
    async def send_action(self, packet, action, machine_id=None, group_id=None):
        ...
        if action['id'] == 'patch':
            for machine_id, machine in machines.items():
                ...
                group_id = machine.config.group.id if machine.config.group else None
        ...
        if action['id'] == 'refresh':
            for machine_id, machine in machines.items():
                ...
                data = await get_group_data(machine_group_id)
